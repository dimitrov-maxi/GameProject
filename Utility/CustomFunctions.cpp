...
#ifdef _WIN32
        IP_ADAPTER_INFO AdapterInfo[16];       // Allocate information for up to 16 adapters
...
int CustomFunctions::startHost() {
        const int port = 5500; // fixed port
        int serverSock = socket(AF_INET, SOCK_STREAM, 0);
        if (serverSock < 0) { std::cerr << "Cannot create socket\n"; return -1; }
        ...
        closeSocket(serverSock); // Only need the client socket
        return clientSock;
    }
...
void CustomFunctions::sendAction(int sock, short action) {
        uint16_t networkValue = htons(static_cast<uint16_t>(action)); // convert to network byte order
...
void CustomFunctions::Fight(Player p, Player p2,bool firstTurn, int socket, Action* playerActions[5]) {
        ...
        while (p.isAlive() && p2.isAlive()) {
            ...
            if (firstTurn) {
                ...
                CustomFunctions::sendAction(socket, choice-1);
                firstTurn = false;
            }else{
                p.printStats();
                p2.printStats();
                std::cout << "Waiting for enemy turn"<<std::endl;
                int action = CustomFunctions::receiveAction(socket);
